"""Frozen Qwen3.5-9B single-GPU scale-transfer evaluation."""
...
PROTOCOL_SCHEMA = "hmo.contiguous_cf.scale_transfer_protocol.v1"
RESULT_SCHEMA = "hmo.contiguous_cf.scale_transfer_result.v1"
RESULTS_FILENAME = "scale_transfer_results.jsonl"
SUMMARY_FILENAME = "scale_transfer_summary.json"
SYSTEMS = (
    "contiguous_cf",
...
def load_scale_transfer_protocol(path: Path) -> tuple[dict, str]:
    try:
        encoded = path.read_bytes()
        payload = json.loads(encoded)
    except (OSError, json.JSONDecodeError) as exc:
        raise OracleContractError(
            f"cannot read scale-transfer protocol: {path}"
        ) from exc
    ...
    if (
        ...
        or shapes.get("smoke", ())[:5]
        != ("needle", 1, 8192, 256, 0.1)
        or shapes.get("8k", ())[:5]
        != ("needle,longeval_lines", 6, 8192, 256, 0.1)
        or shapes.get("16k", ())[:5]
        != ("needle,longeval_lines", 6, 16384, 256, 0.1)
        or any(shape[5] <= 0 or not shape[6] for shape in shapes.values())
        or len({shape[5] for shape in shapes.values()}) != len(shapes)
        or len({shape[6] for shape in shapes.values()}) != len(shapes)
    ):
        raise OracleContractError("scale-transfer protocol mismatch")
    ...
    with path.open(encoding="utf-8") as handle:
        for line_number, line in enumerate(handle, start=1):
            ...
            try:
                ...
                raise OracleContractError(
                    f"invalid scale-transfer result at line {line_number}"
                ) from exc
    keys = [(row.get("stage"), row.get("sample_id")) for row in rows]
    if len(keys) != len(set(keys)):
        raise OracleContractError("scale-transfer results contain duplicate cases")
    return rows


def summarize_scale_transfer_results(rows: Sequence[Mapping]) -> dict:
    if not rows:
        raise OracleContractError("scale-transfer summary requires result rows")
...
def run(args: argparse.Namespace) -> dict:
    if not torch.cuda.is_available() or torch.cuda.device_count() != 1:
        raise RuntimeError("scale transfer requires exactly one visible GPU")
    protocol, protocol_sha = load_scale_transfer_protocol(
    ...
    if (
        ...
        raise OracleContractError(
            "model identity disagrees with scale-transfer protocol"
        )
    ...
    manifest = ensure_run_manifest(
        run_dir,
        experiment="contiguous_coverage_fidelity_scale_transfer",
        args={
        ...
        selections={
            "scope": (
                ...
                else "frozen_larger_model_transfer_no_retuning"
            ),
    ...
    if completed and not args.resume:
        raise OracleContractError(
            "scale-transfer results exist; pass --resume to continue"
        )
    ...
    if not attention_layers or not recurrent_layers:
        raise OracleContractError(
            "scale-transfer model is not a hybrid attention/recurrent architecture"
        )
    ...
    if len(cases) != expected_case_count or len(
        ...
        raise OracleContractError("scale-transfer sample construction changed")
    ...
    for case_index, (stage, sample) in enumerate(cases, start=1):
        ...
        if exact_plan.total_charged_bytes != sparse_plan.total_charged_bytes:
            raise OracleContractError(
                "scale-transfer compressed plans are not equal-byte"
            )
        ...
        for system, intervention in arm_specs:
            ...
            resident_bytes = get_active_kv_bytes(
                state.cache, list(attention_layers)
            )
        ...
        if len(equal_resident) != 1:
            raise OracleContractError(
                "scale-transfer equal-byte arms differ in resident KV"
            )
        ...
        for name in EQUAL_BYTE_SYSTEMS:
            if generated[name]["post_query_resident_kv_bytes"] != compressed_expected:
                raise OracleContractError(
                    f"{name} post-query bytes disagree with equal-byte target"
                )
        ...
        raw_expected = (
            protected_bytes
            + raw_selection["budget_slots"]
            * raw_selection["unit_segment_bytes"]
            + query_kv_bytes
        )
        full_expected = (
        ...
        if (
            generated["raw_alpha_exact_topk"][
                "post_query_resident_kv_bytes"
            ]
            != raw_expected
            or generated["full_kv_reference"][
                "post_query_resident_kv_bytes"
            ]
            != full_expected
        ):
            raise OracleContractError(
                "scale-transfer reference byte accounting failed"
            )
        ...
        print(
            ...
            f"raw+slack={generated['raw_alpha_exact_slack']['normalized_answer_contains']:.0f} "
    ...
    if {(row["stage"], row["sample_id"]) for row in rows} != expected_keys:
        raise OracleContractError(
            "scale transfer did not complete the selected frozen sample set"
        )
    rows.sort(
        key=lambda row: (row["stage"], row["dataset"], row["sample_id"])
    )
    summary = {
        ...
        "protocol_sha256": protocol_sha,
        "analysis": summarize_scale_transfer_results(rows),
        "runtime": {
